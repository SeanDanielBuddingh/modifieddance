    ...
    def load_data(self):
        ...
        for i, g_name in enumerate(brain_train.iloc):
            ...
            for x, exp in enumerate(sorted.iloc):
                ...
                prelim_brain_train = pd.concat([prelim_brain_train, pd.Series([c_name] * exp.values)], axis=0)
            corpus_train_brain.append(prelim_brain_train.tolist())

        for i, g_name in enumerate(brain_test.iloc):
            prelim_brain_test = pd.Series()
            gene_exp = brain_test.iloc[i, :]
            sorted = pd.DataFrame(gene_exp[gene_exp!=0].sort_values(ascending=False))
            #sorted = sorted.T
            for x, exp in enumerate(sorted.iloc):
                c_name = sorted.index.tolist()[x]
                prelim_brain_test = pd.concat([prelim_brain_test, pd.Series([c_name] * exp.values)], axis=0)
            corpus_test_brain.append(prelim_brain_test.tolist())
