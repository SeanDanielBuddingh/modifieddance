...
class data_pre():
    def __init__(self):
        ...
        #self.get_w2v(brain_x, corpus_brain, spleen_x, corpus_spleen, kidney_x, corpus_kidney, brain_y, spleen_y, kidney_y, b_train, corpus_btrain, s_train, corpus_strain, k_train, corpus_ktrain, btrain_y, strain_y, ktrain_y)
    ...
    def load_data(self):
        ...
        b_train = pd.concat([pd.read_csv(self.path+'/train/mouse/mouse_Brain753_data.csv', header=0, index_col=0),
                             pd.read_csv(self.path+'/train/mouse/mouse_Brain3285_data.csv', header=0, index_col=0)],axis=1, ignore_index=False).fillna(0)
        b_train.columns = range(b_train.shape[1])
        #print(b_train.index)
    ...
    def get_w2v(self, brain_x, corpus_brain, spleen_x, corpus_spleen, kidney_x, corpus_kidney, brain_y, spleen_y, kidney_y,
        ...
        b_cells = b_train.T.values @ b_matrix
        b_cells = pd.DataFrame(b_cells)
        b_cells.to_csv(self.path+'/b_train.csv', index=False, header=False)
        ...
        s_cells = s_train.T.values @ s_matrix
        s_cells = pd.DataFrame(s_cells)
        s_cells.to_csv(self.path+'/s_train.csv', index=False, header=False)
        ...
        k_cells = k_train.T.values @ k_matrix
        k_cells = pd.DataFrame(k_cells)
        k_cells.to_csv(self.path+'/k_train.csv', index=False, header=False)
        ...
        embeddings.to_csv(self.path+'/train_embeddings.csv', index=False)
        ...
        brain_cells = brain_x.T.values @ brain_matrix
        brain_cells = pd.DataFrame(brain_cells)
        brain_cells.to_csv(self.path+'/brain_cells.csv', index=False, header=False)        
        ...
        spleen_cells = spleen_x.T.values @ spleen_matrix
        spleen_cells = pd.DataFrame(spleen_cells)
        spleen_cells.to_csv(self.path+'/spleen_cells.csv', index=False, header=False)        
        ...
        kidney_cells = kidney_x.T.values @ kidney_matrix
        kidney_cells = pd.DataFrame(kidney_cells)
        kidney_cells.to_csv(self.path+'/kidney_cells.csv', index=False, header=False)        
