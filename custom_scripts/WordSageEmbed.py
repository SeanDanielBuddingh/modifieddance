...
def read_data(batch_size, seed, dataset):
    data = data_pre()
    tissue_train, tissue_test, genes, y_values_train, y_values_test, normalized_train, normalized_test, train_raw, test_raw = data.read_w2v(dataset)
    ...
    return train_graphs, bce_targets_train_list, targets_train_list, test_graphs, bce_targets_test_list, targets_test_list, train_nodes_list, test_nodes_list, targets_encoded_train, targets_encoded_test
...
for dataset in datasets:
    ...
    targets_encoded_train_np = targets_encoded_train.to_numpy()
    targets_encoded_test_np = targets_encoded_test.to_numpy()
    ...
    if not os.path.exists(data_dir_+"/y_hat_"+dataset+"_train.csv"):
        y_hat_list = []
        for epoch in range(10):
            ...
            for i, (train_graph, bce_targets_train, train_targets, train_nodes) in enumerate(zip(train_graphs, bce_targets_train_list, targets_train_list, train_nodes_list)):
                ...
                optimizer.step()
                if epoch == 9:
                    y_hat_list.append(torch.sigmoid(bce_cells)) # sigmoid for probabilities
        ...
        y_hat_df.to_csv(data_dir_+"/y_hat_"+dataset+"_train.csv", index=False, header=False)

    else:
        y_hat_df = pd.read_csv(data_dir_+"/y_hat_"+dataset+"_train.csv", header=None)
    ...
    features_df.to_csv(data_dir_+"/tuned_"+dataset+"_train.csv",index=False, header=False)
    targets_df.to_csv(data_dir_+"/tuned_"+dataset+"_train_labels.csv", index=False, header='Cell_Type')

    # Reinitialize optimizer for block 2 (HARD TARGETS AKA NO BLOCK 1)
    block2 = WordSAGEBLOCK2((src_dim, dst_dim2), hidden_channels, out_channels, num_classes, num_binary_targets).to(device)
    del bce_loss, ce_loss, optimizer
    torch.cuda.empty_cache() 
    gc.collect()
    ...
    for epoch in range(50):
        ...
        for i, (train_graph, bce_targets_train, train_targets, train_nodes) in enumerate(zip(train_graphs, bce_targets_train_list, targets_train_list, train_nodes_list)):
            ...
            optimizer.zero_grad()
            feature, out_cells = block2(train_graph, train_feature_map, torch.tensor(bce_targets_train.to_numpy(),dtype=torch.long).to(device))
            ce_loss_value = ce_loss(out_cells, train_targets.squeeze(1))
            for feat_out, target_out in zip(out_cells, train_targets.squeeze(1)):
                feat_out = F.softmax(feat_out, dim=0)
                feat_out = torch.argmax(feat_out)
    ...
    features_df.to_csv(data_dir_+"/tuned_"+dataset+"_hard_train.csv",index=False, header=False)
    targets_df.to_csv(data_dir_+"/tuned_"+dataset+"_hard_train_labels.csv", index=False, header='Cell_Type')    
    ...
    if not os.path.exists(data_dir_+"/y_hat_"+dataset+"_test.csv"):
        y_hat_list = []
        for epoch in range(10):
            features_list = []
            targets_list = []
            ...
            for i, (test_graph, bce_targets_test, test_targets, test_nodes) in enumerate(zip(test_graphs, bce_targets_test_list, targets_test_list, test_nodes_list)):
                ...
                optimizer.step()
                if epoch == 9:
                    y_hat_list.append(torch.sigmoid(bce_cells)) # sigmoid to make them into probabilities
        ...
        y_hat_dfs = [pd.DataFrame(tensor.cpu().detach().squeeze(1).numpy()) for tensor in y_hat_list]
        y_hat_df = pd.concat(y_hat_dfs, ignore_index=True)
        y_hat_df.to_csv(data_dir_+"/y_hat_"+dataset+"_test.csv", index=False, header=False)

    else:
        y_hat_df = pd.read_csv(data_dir_+"/y_hat_"+dataset+"_test.csv", header=None)
        y_hat_list = [torch.tensor(y_hat_df.iloc[i*batch_size:min((i+1)*batch_size, len(y_hat_df))].values, dtype=torch.float32).to(device) for i in range(len(y_hat_df)//batch_size + 1)]
    ...
    features_df.to_csv(data_dir_+"/tuned_"+dataset+"_test.csv",index=False, header=False)
    targets_df.to_csv(data_dir_+"/tuned_"+dataset+"_test_labels.csv", index=False, header='Cell_Type')
    ...
    for epoch in range(50):
        ...
        for i, (test_graph, bce_targets_test, test_targets, test_nodes) in enumerate(zip(test_graphs, bce_targets_test_list, targets_test_list, test_nodes_list)):
            ...
            optimizer.zero_grad()
            feature, out_cells = block2(test_graph, test_feature_map, torch.tensor(bce_targets_test.to_numpy(),dtype=torch.long).to(device))
            ce_loss_value = ce_loss(out_cells, test_targets.squeeze(1))
            for feat_out, target_out in zip(out_cells, test_targets.squeeze(1)):
                feat_out = F.softmax(feat_out, dim=0)
                feat_out = torch.argmax(feat_out)
    ...
    features_df.to_csv(data_dir_+"/tuned_"+dataset+"_hard_test.csv",index=False, header=False)
    targets_df.to_csv(data_dir_+"/tuned_"+dataset+"_hard_test_labels.csv", index=False, header='Cell_Type')
